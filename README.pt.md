...
- **Documentação que não pode mentir.** A documentação de referência é gerada directamente a partir do CSS compilado, nunca escrita à mão. Se uma classe existe no build, está documentada. Se não existe, não está. Sem divergência entre o que é publicado e o que está escrito.
...
Ver [CONTRIBUTING.md](./CONTRIBUTING.md). O projecto ainda não aceita contribuições de código externas — ainda não há uma API pública estável para contribuir — mas issues e discussão são bem-vindas.
