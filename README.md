...
- **Documentation that can't lie.** Reference docs are generated directly from the compiled CSS output, not hand-written. If a class exists in the build, it's documented. If it doesn't, it isn't. No drift between what's shipped and what's written down.
...
See [CONTRIBUTING.md](./CONTRIBUTING.md). The project isn't accepting external code contributions yet — there's no stable public API to contribute against — but issues and discussion are welcome.
